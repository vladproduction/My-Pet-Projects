...
public class ElectronicWatchAdvanceTest {

    @ParameterizedTest
    @MethodSource("testCases")
    public void testElectronicWatchAdvance(int input, String expected) {
        ...
        PrintStream controlledOut = new PrintStream(byteArrayOutputStream);
        PrintStream defaultOut = System.out;

        try{
            System.setIn(controlledIn);
            System.setOut(controlledOut);
            ElectronicWatchAdvance.main(new String[]{});
            controlledOut.flush();
            assertEquals(expected, byteArrayOutputStream.toString().trim(), "Error on input value: " + input);
        } finally {
